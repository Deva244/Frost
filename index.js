...
bot.on('message', async msg => {
  ...
  if (msg.content.startsWith(`${prefix}`)){
    ...
    if (content.includes('kira') && rate >= 70) {
      msg.channel.send(copypasta.kira);
      return;
    }
    if (content.includes("jojo reference") || content.includes('jonathan') || content.includes('dio') && rate >= 70) {
      msg.channel.send(copypasta.reference);
      return;
    }
    if (content.includes('approach') || content.includes('closer') || content.includes("beat") && rate >= 70) {
      msg.channel.send(copypasta.approach);
      return;
    }
    if (content.includes('rant') || content.includes('venice') || content.includes("france") || content.includes('paris') && rate >= 70) {
      msg.channel.send(copypasta.venice);
      return;
    }
    if (content.includes('enemy') || content.includes('stand') || content.includes("care") && rate >= 70) {
      msg.channel.send(copypasta.enemy);
      return;
    }
    if (content.includes('heaven') || content.includes('priest') || content.includes("pope") || content.includes('papa') && rate >= 70) {
      msg.channel.send(copypasta.heaven);
      return;
    }
    if (content.includes('watch') || content.includes('jojo') && rate >= 70) {
      msg.channel.send(copypasta.jojo);
      return;
    }
    if (content.includes('door') && rate >= 70) {
      let embed = new Discord.RichEmbed().setImage('https://i.imgur.com/Hi0YNft.png').setColor('#d7691a');
      msg.channel.send(embed);
      return;
    }
    if (content.includes('yo') || content.includes('rock') || content.includes('angelo') && rate >= 70) {
      let embed = new Discord.RichEmbed().setImage('https://i.imgur.com/NOOhJ9l.jpg').setColor('#8f8e8e');
      msg.channel.send(embed);
      return;
    }
    if (content.includes('torture') || content.includes('dance') && rate >= 70) {
